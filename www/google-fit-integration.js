...
async function initGoogleFitNative() {
    ...
    try {
        // Check if GoogleFit plugin is available
        if (!Capacitor.Plugins.GoogleFit) {
            console.warn('⚠️ Google Fit plugin not installed - using simulated data');
            startSimulatedHealthData();
            return;
        }
        ...
        if (result.granted) {
            ...
            console.error('❌ Google Fit permissions denied - using simulated data');
            startSimulatedHealthData();
        }
    } catch (error) {
        console.error('Error initializing Google Fit:', error);
        console.log('🔄 Falling back to simulated health data');
        startSimulatedHealthData();
    }
}
...
// Start simulated health data (for demo/testing)
function startSimulatedHealthData() {
    console.log('🎯 Starting simulated health data (demo mode)...');
    ...
    function updateSimulatedData() {
        ...
        healthData.steps = Math.floor(2000 + Math.random() * 8000); // 2000-10000 steps
        healthData.heartRate = Math.floor(60 + Math.random() * 40); // 60-100 bpm
        healthData.calories = Math.floor(healthData.steps * 0.04); // ~0.04 cal per step
        healthData.sleep = hour < 6 ? Math.floor(360 + Math.random() * 120) : 0; // 6-8 hours in minutes
        healthData.lastUpdate = Date.now();
        
        console.log('📊 Simulated health data updated:', healthData);
        
        // Update trigger state
        if (typeof triggerState !== 'undefined') {
            triggerState.healthDataLastSync = Date.now();
            triggerState.lastActivityTime = Date.now() - (healthData.steps > 0 ? 0 : 2 * 60 * 60 * 1000);
        }
    ...
    setInterval(updateSimulatedData, 5 * 60 * 1000);
}
